...
# This script is the same as allocate_egs.py, but is modified to support multi-speaker
...
def get_labels(utt2int_filename):
    ...
    for line in f:
        ...
        spk = tokens[1] # can be of the form int or int,int
        utt = tokens[0]
        if "," in spk:
            spk1, spk2 = spk.split(",")
            spk2utt[spk1].append(utt)
            spk2utt[spk2].append(utt)
            utt2spk[utt] = [spk1, spk2]
        else:
            spk2utt[spk].append(utt)
            utt2spk[utt] = [spk, "-1"]  # pad with -1
    f.close()
...
def splitting(args, spk2utt, utt2spk, utt2len, prefix):
    ...
    pdf2num = {}    # count number of examples per label
    cur_archive = 0
    for job in range(args.num_jobs):
        ...
        for (utterance_index, i, offset) in sorted(this_ranges):
            ...
            print("{0} {1} {2} {3} {4} {5}".format(utterance_index,
                                                   ...
                                                   offset,
                                                   frames_per_chunk,
                                                   " ".join(utt2spk[utterance_index])),
                  file=f)
            spk = utt2spk[utterance_index]
            pdf2num[spk[0]] = pdf2num.get(spk[0], 0) + 1
            if spk[1] != "-1":
                pdf2num[spk[1]] = pdf2num.get(spk[1], 0) + 1
        f.close()
