...
def verify_egs_dir(egs_dir):
    try:
        feat_dim_path = os.path.join(egs_dir, 'info', 'feat_dim')
        assert os.path.isfile(feat_dim_path), f'File does not exist: `{feat_dim_path}`'
        egs_feat_dim = int(open(feat_dim_path).readline())

        num_archives_path = os.path.join(egs_dir, 'info', 'num_archives')
        assert os.path.isfile(num_archives_path), f'File does not exist: `{num_archives_path}`'
        num_archives = int(open(num_archives_path).readline())

        # implementation slightly changed to read number of line in egs scp
        egs_scp_path = os.path.join(egs_dir, 'egs.1.scp')
        assert os.path.isfile(egs_scp_path), f'File does not exist: `{egs_scp_path}`'
        with open(egs_scp_path) as f:
            num_egs_in_scp = len(f.readlines())

        return [num_archives, egs_feat_dim, num_egs_in_scp]
    except (IOError, ValueError):
        logger.error(f'The egs dir {egs_dir} has missing or malformed files.')
        raise
